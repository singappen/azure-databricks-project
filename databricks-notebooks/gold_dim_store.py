...
if not spark.catalog.tableExists('elesales_catalog.gold.dim_store'):
    df_sink = spark.sql(
        '''
        SELECT 1 as dim_store_key, Store_ID, StoreName 
        FROM PARQUET.`abfss://silver@azstorageaccountstunity.dfs.core.windows.net/electronicssales`
        WHERE 1=0
        '''
    )
    ...
    df_sink = spark.sql(
        '''
        SELECT dim_branch_key, Store_ID, StoreName
        FROM elesales_catalog.gold.dim_store
        '''
    )
...
df_filter = df_src.join(df_sink, df_src.Store_ID == df_sink.Store_ID, 'left').select(df_src.Store_ID, df_src.StoreName, df_sink.dim_store_key)
...
df_filter_old = df_filter.filter(df_filter.dim_store_key.isNotNull())
...
df_filter_new = df_filter.filter(df_filter.dim_store_key.isNull()).select('Store_ID', 'StoreName')
...
if (incremental_flag == '0'): 
    ...
    if spark.catalog.tableExists('elesales_catalog.gold.dim_store'):
        max_value_df = spark.sql("SELECT max(dim_store_key) FROM elesales_catalog.gold.dim_store")
...
df_filer_new = df_filter_new.withColumn('dim_store_key', max_value + F.monotonically_increasing_id())
...
df_filter_new = df_filter_new.withColumn('dim_store_key', max_value + F.monotonically_increasing_id())
...
# Incremental RUN 
if spark.catalog.tableExists('elesales_catalog.gold.dim_store'):
    delta_table = DeltaTable.forPath(spark, "abfss://gold@azstorageaccountstunity.dfs.core.windows.net/dim_store")
    # update when the value exists
    # insert when new value 
    delta_table.alias("target").merge(df_final.alias("source"), "target.dim_store_key = source.dim_store_key")\
...
# Initial RUN 
    ...
    df_final.write.format("delta")\
        .mode("overwrite")\
        .option("path", "abfss://gold@azstorageaccountstunity.dfs.core.windows.net/dim_store")\
        .saveAsTable("elesales_catalog.gold.dim_store")

# COMMAND ----------

# MAGIC %sql 
# MAGIC SELECT * FROM elesales_catalog.gold.dim_store
